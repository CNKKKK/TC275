...
void TIM3_Init(u16 arr,u16 psc)//定时器3中断初始化
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	//定时器中断3初始化
	TIM_TimeBaseStructure.TIM_Period = arr; //更新在下一个装入活动的自动重装寄存器周期的值
	TIM_TimeBaseStructure.TIM_Prescaler =psc; //设置用来作为TIMx时钟频率除数的预分值
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1; //设置时钟分割频率TDTS = Tck_tim
