...
//for(i=0;i<7;i++)
//			{
//				if(usart1Buf[i*7]==0x0b)//筛选出地址是0b的信息 存到内存准备发送
//				{	}//把UASRT2收到的内容通过UASRT1发过去,以7位为单位发送

//			}					
	myUSART_Sendarr(USART1,asd,7);
		while(USART_GetFlagStatus(USART1,USART_FLAG_TC)==RESET);//发送缓冲区标志为空
...
void USART2_IRQHandler(void)//UART2接收数据函数中断
	...
	{int i;
		while(USART_GetFlagStatus(USART2,USART_FLAG_TXE)==RESET);//发送缓冲区标志为空
 		for(i=0;i<15;i++)
			{
				if(usart1Buf[i*8]==0x0b&&usart1Buf[i*8+3]==0x0c)//筛选出地址是0b的信息 存到内存准备发送
				{
